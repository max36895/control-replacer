...
function getScriptParam() {
  ...
  log(
    "\t- fixCommit - откатывает коммит. Стоит использовать в том случае, если нужно повторно запустить скрипт для создания mr."
  );
  log("");
}
...
if (argv[2]) {
  if (argv[2].indexOf(".json") !== -1) {
    if (FileUtils.isFile(argv[2])) {
      const param: IParam<IReplace> = JSON.parse(FileUtils.read(argv[2]));
      if (param.path) {
        script.run(param);
      } else {
